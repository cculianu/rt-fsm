...
static void detectInputEvents(FSMID_t); /**< sets bitfield array rs[f].events_bits of all the events detected -- each bit corresponds to a state matrix "in event column" position, eg center in is bit 0, center out is bit 1, left-in is bit 2, etc */
...
static void processSchedWaves(FSMID_t); /**< updates active wave state, does output, sets event id mask of any waves that generated input events (if any) in rs[f].events_bits */
static void processSchedWavesAO(FSMID_t); /**< updates active wave state, does output, sets event id mask of any waves that generated input events (if any) in rs[f].events_bits */
...
static void reconfigureIO(void)
{
  ...
  for (f = 0; f < NUM_STATE_MACHINES; ++f) {
    ...
    for (i = FIRST_IN_CHAN(f); i < FIRST_IN_CHAN(f)+NUM_IN_CHANS(f); ++i)
        ...
        if (ss->states->has_sched_waves) { /* Print Sched. Wave statistics */
          ...
          for (i = 0; i < FSM_MAX_SCHED_WAVES; ++i) {
            if (ss->states->sched_waves[i].enabled) {
              seq_printf(m, "DIO Sched. Wave %d  edge_up_col: %d  edge_down_col: %d  (%s)\n", i, ss->states->routing.sched_wave_input[i*2], ss->states->routing.sched_wave_input[i*2+1], ss->active_wave_mask & 0x1<<i ? "playing" : "idle");
            }
          }
        }
        vfree(ss);
  ...
  while (! rt_task_stop) {
    ...
    for (f = 0; f < NUM_STATE_MACHINES; ++f) {
      ...
      if ( rs[f].valid ) {
        ...
        for (n_evt_loops = 0, evt_id = 0; (evt_id = find_next_bit((const unsigned long *)rs[f].events_bits, NUM_INPUT_EVENTS(f), evt_id)) < NUM_INPUT_EVENTS(f) && n_evt_loops < NUM_INPUT_EVENTS(f); ++n_evt_loops, ++evt_id) {
...
static int gotoState(FSMID_t f, unsigned state, int event_id)
{
  ...
  if (state == READY_FOR_TRIAL_JUMPSTATE(f) && rs[f].ready_for_trial_flg) {
    ...
    state = 0;
    stopActiveWaves(f); /* since we are starting a new FSM, force any
...
static void detectInputEvents(FSMID_t f)
{
  ...
  for (i = FIRST_IN_CHAN(f); i < AFTER_LAST_IN_CHAN(f); ++i) {
    ...
    if (event_id_edge_down > -1 /* input event is actually routed somewhere */
      ...
      set_bit(event_id_edge_down, rs[f].events_bits);  /* Return the event id */		   
  }
}
...
static inline long Micro2Sec(long long micro, unsigned long *remainder)
{
  ...
  unsigned long rem;
  if (neg) micro = -micro;
  long ans = ulldiv(micro, MILLION, &rem);
  if (neg) ans = -ans;
