...
#else
#  error This variant of TCC is for kernel-side Linux only!
#endif
...
{
    error("open unimplemented in this build");
    return 0;
}
...
{
    (void) bf;
    error("close unimplemented in this build");    
}
...
static int tcc_peekc_slow(BufferedFile *bf)
{
    ...
    if (bf->buf_ptr >= bf->buf_end) {
        if (bf->fd != -1) {
            ...
            error("tcc_peekc_slow cannot read an actual file, unimplemented in this build");
            len = -1;
            if (len < 0)
                len = 0;
        } else {
                           ...
                           Sym **nested_list, Sym *s, struct macro_level **can_read_stream)
{
    ...
    if (tok == TOK___LINE__) {
      ...
      error("Date/time MACROS unimplemented in this build");
      cstrval = buf;
      buf[0] = 0;
/*         time_t ti; */
/*         struct tm *tm; */
...
int tcc_run(TCCState *s1, int argc, char **argv)
{
    ...
    if (do_debug) {
        error("debug mode currently not available in this embedded build");
    }
