...
extern "C" {
  ...
  L22LINKAGE unsigned short LynxStartup(LinuxContext *ctx) { 
...
#   endif
  ...
  L22LINKAGE void LynxCleanup(LinuxContext *ctx) 
  { 
    if (!ctx) return;
...
//     }
  ...
  L22LINKAGE unsigned short LynxPlay(LinuxContext *ctx, unsigned chan)
  {
  ...
  L22LINKAGE unsigned short LynxStop(LinuxContext *ctx, unsigned chan)
  {
    if (!ctx) return HSTATUS_INVALID_PARAMETER; // paranoia..
    if (!ctx->priv) {
  ...
  L22LINKAGE void LynxPresetSamplingRate(LinuxContext *ctx)
  {
  ...
  L22LINKAGE ULONG LynxGetVolume(LinuxContext *ctx, unsigned chan)
  {
    ULONG volL = 0, volR = 0;
  ...
  L22LINKAGE int LynxSetVolume(LinuxContext *ctx, unsigned chan, ULONG volume)
  {
  ...
  L22LINKAGE int LynxIsPlaying(LinuxContext *ctx, unsigned chan)
  {
    if (ctx && ctx->priv && ctx->priv->adapter 
  ...
  L22LINKAGE int LynxMixerOverflowed(LinuxContext *ctx) 
  {
    if (ctx && ctx->priv && ctx->priv->adapter) {
      PHALMIXER mix = ctx->priv->adapter->GetMixer();
      ULONG val1 = 0, val2 = 0;
...
L22LINKAGE void LynxDoDMAPoll(struct LinuxContext *ctx)
{
  int chan;
  for (chan = 0; chan < L22_NUM_CHANS; ++chan) {
...
L22LINKAGE int LynxIsPlayingSound(LinuxContext *ctx, unsigned chan)
{
