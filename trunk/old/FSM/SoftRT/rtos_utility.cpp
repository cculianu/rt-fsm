...
#define _GNU_SOURCE
#endif
#define MBUF_EMUL_USE_NAMESPACE
#include "mbuffemul.h"
#include "mbuffemul.cpp"
...
void *RTOS::shmAttach(const char *SHM_NAME, size_t size, ShmStatus *s)
{
  ...
  //  if (rtos != Unknown) {
    ...
    switch(rtos) {
      case Unknown:  ret = MbufEmul::mbuff_attach(SHM_NAME, size);      break;
...
void RTOS::shmDetach(const void *SHM, ShmStatus *s)
{
  ...
  if ( it != shmMap.end() ) {
      ...
      switch (inf.rtos) {
        case Unknown:  MbufEmul::mbuff_detach(inf.name.c_str(), inf.address); break;
...
int RTOS::openFifo(int minor_no, ModeFlag mode)
{
  ...
  if (determine() == Unknown) 
    return rtf_open(minor_no);
